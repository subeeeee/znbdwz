<template>
  <div id='MarketingTeamManage' class="marketingTeamManage">
    <el-row>
          ...
          <el-popover
            ...
            <div style="text-align: center; margin: 0 auto">
              <el-upload
                ...
                action="/iCloud-rest/projects/expandTeams/excel/import"
      ...
      <el-col :span="24" class='teamsSaleManageList'>
        <el-col :span="2" class="title">拓客总监：</el-col>
        <el-col :span="22">
          <div class="salesBox">
      ...
      <el-col :span="17" :offset="1" class="steamManagerBox" v-if="saleManageList && groupList">
        <el-col :span="24" class="steamManager">
          <el-col :span="3" class="title">团长：</el-col>
          <el-col :span="15">
            <div class="salesBox">
        ...
        <el-col :span="24" v-if="groupList.length" v-for="group in groupList" :key="group.groupId">
          ...
          <el-col :span="24" class="steamGroupings">
            <el-col :span="3" class="title">组长：</el-col>
            <el-col :span="20">
              <div class="salesBox">
          ...
          <el-col :span="24" class="steamGroupings">
            <el-col :span="3" class="title">组员：</el-col>
            <el-col :span="20">
              <div class="salesBox">
    ...
    <el-dialog ref='dialog' :title="title" :visible.sync="dialogFormVisible" top="20vh" @close="dialogClose">
      ...
      <el-row v-if="isSalesGroup || isPutSalesGroup">
        <el-form :label-position="labelPosition" :model="salesGroup" :rules="salesGroupRules" ref="salesGroupForm" >
          <el-col :span="14">
            <el-form-item label="拓客组名称：" :label-width="formLabelWidth" prop="salesGroupName">
...
<script>
  import Vue from 'vue'
  import {
    requestExpandTeamExcelExport,
    requestExpandTeamExcelProcess,
    requestExpandTeamDirectors,
    requestGetExpandTeamMembers,
    requestPostExpandTeamsMember,
    requestGetExpandTeam,
    requestGetExpandGroupsList,
    requestPostExpandTeam,
    requestPutExpandTeam,
    requestPutExpandTeamsMembers,
    requestDeleteExpandTeam,
    requestGetExpandTeamsStructure,
    requestDeleteExpandTeamMembers,
    requestGetExpandGroupsMembers,
    requestGetExpandMembers
  ...
  export default {
    ...
    data () {
      return {
        ...
        salesGroupRules: {
          salesGroupName: [
            {required: true, message: '请输入拓客组名称', trigger: 'blur'}
          ]
        },
        selectMember: {
          memberName: ''
    ...
    methods: {
      ...
      async getTeams (firstLoad) {
        ...
        if (this.params.projectId) {
          const result = await requestGetExpandTeam(this.params)
      ...
      async getTeamsGroup () {
        ...
        if (this.params.projectId) {
          const result = await requestGetExpandGroupsList(params)
      ...
      async postTeams () {
        ...
        if (this.params.projectId) {
          ...
          await requestPostExpandTeam(this.teamsParams)
          this.getTeams()
        } else {
          this.$message.info('请先选择项目')
        }
      ...
      async putTeams () {
        ...
        this.teamsParams.name = this.salesTeam.salesTeamName
        this.teamsParams.type = 0
        await requestPutExpandTeam(this.teamsParams, this.teamsParams.teamId)
        this.getTeams()
        this.dialogFormVisible = false
      },
      async putGroup () {
      ...
      async postTeamsMember () {
        if (this.SelectMemberData.length) {
          ...
          await requestPostExpandTeamsMember(this.addMember)
          this.identity === 3 ? this.getTeamsSaleManagers() : this.loadGroup()
          this.dialogFormVisible = false
      ...
      async teamsMembersPut () {
        if (this.SelectMemberData.length) {
          ...
          if (this.putOrganizationParams.length) {
            await requestPutExpandTeamsMembers(this.putOrganizationParams)
      ...
      async getTeamsMember () {
        ...
        const result = await requestGetExpandMembers(params)
        this.tableData = result.data
      },
      // 获取为选择成员列表
      async getTeamsMembers () {
        this.teamsMembersParams.projectId = sessionStorage.getItem('projectId')
        const result = await requestGetExpandTeamMembers(this.teamsMembersParams)
      ...
      async getTeamsGroupMembers (params) {
        const result = await requestGetExpandGroupsMembers(params)
        this.tableData = result.data
      },
      // 获取团队销售经理和分接
      async getTeamsSaleManagers () {
        this.params.projectId = sessionStorage.getItem('projectId')
        const result = await requestExpandTeamDirectors(this.params)
        this.teamsSaleManageList = result.data
      },
      ...
      // 导出模板
      exportTemplate () {
        this.downloadByIframe('./static/excelTemplate/行销团队导入模板.xlsx')
      },
      async exportExcel () {
        const form = {
          projectId: sessionStorage.getItem('projectId'),
        ...
        if (!form.projectId) {
          ...
          const result = await requestExpandTeamExcelExport(form)
          const date = new Date()
          const currentTime = formatDate(date, 'yyyyMMdd_hhmmss')
          this.globalExport(result, `${this.projectNameActive}_行销团队列表_${currentTime}`)
          this.$message.success('导出行销团队列表成功')
        }
      },
      // 加载分组
      async loadGroup () {
        ...
        const result = await requestGetExpandTeamsStructure(params, this.group.teamId)
        this.groupList = result.data.groupList
        this.saleManageList = result.data.managerList
      },
      ...
      deleteTeams (items, title, type) {
        this.$confirm(`<h3>${title}</h3>`, type === 0 ? '删除团队' : '删除小组', {
          ...
          await requestDeleteExpandTeam(params, params.teamId)
          if (type === 0) {
            this.getTeams()
      ...
      removeSteam (data, title) {
        this.$confirm(`<h3>${title}</h3><p style="margin: 10px 0;">如果当前用户已离职，建议在成员管理置为离职，会同时触发移除各团队</p><p>如果仅仅是调岗，请继续确定移出当前团队</p>`, '移出团队', {
          ...
          await requestDeleteExpandTeamMembers(params, data.memberId)
          this.getTeamsSaleManagers()
          if (this.group.teamId) {
            this.loadGroup()
          }
      ...
      handleSuccess (res, file, fileList) {
        if (res.status === 200) {
          ...
          this.timerId = setInterval(async() => {
            const result = await requestExpandTeamExcelProcess(res.data)
            const state = {
          ...
          if (!this.newGroup.length) {
            ...
            while (i--) {
              if (selection[i].identity === 0 || selection[i].identity === 1) {
                this.$refs.memberTable.toggleRowSelection(selection[i], false)
                this.$message.info('组长和组员只能调整到指定小组')
                selection.splice(i, 1)
              }
            }
          }
