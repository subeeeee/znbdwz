  ...
  <div class="OSSUpload">
    <slot>
      <div>
        ...
        <div v-if="maxNum===0 || maxNum!==0 && houseType.imageUrls.length<maxNum"
             tabindex="0"
...
export default {
  ...
  methods: {
    ...
    deleteImg (index) {
      this.houseType.imageParse.splice(index, 1)
    ...
    async uploadPatternImage () {
      try {
        ...
        if (files !== undefined) {
          Object.keys(files).map((idx,index) => {
            if(this.maxNum == 0 || index <= (this.maxNum - this.houseType.imageParse.length)){
              let timeStamp = new Date().getTime()
              this.houseType.imageParse.push(this.name + timeStamp + '.jpg')
              let file = files[idx]
