<template>
  <div class="RegisteredUser">
    <div class="search">
      ...
      <el-form :inline="true" :model="search" class="demo-form-inline-on identity">
        <el-form-item label="用户身份：">
          <el-radio-group v-model="search.identity" size="mini">
            <el-radio-button v-for="(item, index) in identityList" :key="index" :label="item.identity">{{item.identityName}}</el-radio-button>
          </el-radio-group>
        </el-form-item>
      </el-form>
      <el-form :inline="true" :model="search" class="demo-form-inline-on">
        ...
        <el-form-item>
          <el-button type="primary" class="search-submit" icon="el-icon-search" @click="getStatisticsGetAgentRankList">搜索</el-button>
        </el-form-item>
      </el-form>
    </div>
    <div class="box"></div>
    ...
    <el-table :data="tableData" class="list-table" style="width:100%;">
      <el-table-column prop="identityname" label="渠道身份"></el-table-column>
...
import { getStatisticsGetAgentRankList, getDownStatisticsGetAgentRankList,getIdentityData } from '../../common/api'
...
export default {
  ...
  methods: {
    ...
    async getIdentityData(){
      let result = await getIdentityData({ type: 5, tenantId: this.search.tenantId })
      if (result.code === 200) {
        let arr = [
          {
            identityName: '全部',
        ...
        this.identityList = [...arr,...result.data]
      }
    },
    // 用户报备统计
    ...
    export2Excel () {
      ...
      require.ensure([], () => {
        ...
        const tHeader = ['渠道身份', '姓名', '全部报备', '未到访', '已到访', '已认筹', '已认购', '已签约', '到访率', '购买率'] // 上面设置Excel的表格第一行的标题
        const filterVal = ['identityname', 'name', 'allNum', 'notVisitNum', 'visitNum', 'recognitionNum', 'subscriptionNum', 'signNum', 'visitProportion', 'buyProportion'] // 上面的index、nickName、name是tableData里对象的属性
